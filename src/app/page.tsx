...
export const metadata: Metadata = {
  title: "Troublio: Error Codes, Causes and Safe Fixes",
  description: "Find the meaning of appliance, phone, Wi-Fi, printer and app error codes. Get likely causes, safe checks and clear guidance on when to call service.",
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <section className="hero">
        <div className="container hero-grid">
          <div className="hero-copy">
            ...
            <h1>Find the error code meaning. <em>Fix the right thing.</em></h1>
            <p>Search the exact code or symptom to see the likely cause, the safest checks to try first, and the point where qualified service is the correct next step.</p>
            <SearchBox />
            <div className="hero-actions"><Link className="button button-primary" href="/decoder">Decode an error code</Link><Link className="button button-secondary" href="/diagnose">Start guided diagnosis</Link></div>
            <div className="search-suggestions"><span>Popular now:</span>{searchDemandGuides.slice(0, 3).map((problem) => <Link href={`/problems/${problem.slug}`} key={problem.slug}>{problem.shortTitle}</Link>)}</div>
          </div>
      ...
      <section className="trust-strip" aria-label="Troublio principles"><div className="container trust-grid"><div className="trust-item"><span>1</span><div><strong>Direct answer first</strong><small>No long introduction before the likely condition.</small></div></div><div className="trust-item"><span>2</span><div><strong>Safe checks in order</strong><small>External observations before parts or service.</small></div></div><div className="trust-item"><span>3</span><div><strong>Official manual links</strong><small>Model-specific documentation remains the deciding reference.</small></div></div></div></section>
      ...
      <section className="section section-muted"><div className="container"><div className="section-heading"><div><span className="eyebrow">High-intent device hubs</span><h2>Start with the thing that is failing.</h2></div><Link className="text-link" href="/devices">View all devices →</Link></div><div className="hub-directory-grid hub-directory-home">{deviceHubs.slice(0, 10).map((hub) => <Link className="hub-directory-card" href={`/devices/${hub.slug}`} key={hub.slug}><span>{getHubProblems(hub).length} guides</span><h3>{hub.name}</h3><p>{hub.description}</p><strong>Open hub →</strong></Link>)}</div></div></section>

      <section className="section"><div className="container"><div className="section-heading"><div><span className="eyebrow">Browse by symptom</span><h2>When there is no useful error code.</h2></div><Link className="text-link" href="/issues">View all issue hubs →</Link></div><div className="issue-chip-grid">{issueHubs.map((hub) => <Link href={`/issues/${hub.slug}`} key={hub.slug}><strong>{hub.name}</strong><span>{getHubProblems(hub).length} guides</span></Link>)}</div></div></section>

      <section className="section section-dark"><div className="container"><div className="section-heading"><div><span className="eyebrow">New search clusters</span><h2>Fresh guides for the problems people type exactly as they happen.</h2></div><Link className="text-link text-link-light" href="/guides">All guides A–Z →</Link></div><div className="problem-grid">{expansionGuides.map((problem) => <ProblemCard key={problem.slug} problem={problem} />)}</div></div></section>

      <section className="section section-muted"><div className="container"><div className="section-heading"><div><span className="eyebrow">Browse by area</span><h2>Problems happen everywhere.</h2></div><p>Each area is organized around symptoms, error codes, and observable behavior.</p></div><div className="category-grid">{categories.map((category) => <CategoryCard key={category.slug} category={category} count={problems.filter((problem) => problem.categorySlug === category.slug).length} />)}</div></div></section>

      <section className="section section-dark"><div className="container"><div className="section-heading"><div><span className="eyebrow">How Troublio works</span><h2>From display or symptom to a useful next step.</h2></div></div><div className="how-grid"><article className="how-card"><h3>Search the exact display</h3><p>Use the brand, appliance, full model number, and code exactly as shown.</p></article><article className="how-card"><h3>Compare observations</h3><p>Choose what the appliance is actually doing, rather than guessing the failed part.</p></article><article className="how-card"><h3>Follow the safe path</h3><p>Try external checks first, then use the official manual or qualified service.</p></article></div></div></section>

      <section className="section"><div className="container"><div className="section-heading"><div><span className="eyebrow">Popular starting points</span><h2>Common codes, clearer paths.</h2></div><Link className="text-link" href="/error-codes">Browse all error codes →</Link></div><div className="problem-grid">{featured.map((problem) => <ProblemCard key={problem.slug} problem={problem} />)}</div></div></section>

      <section className="section-tight"><div className="container submit-card"><div className="submit-copy"><span className="eyebrow">Missing a problem?</span><h2>Tell Troublio what should be solved next.</h2><p>Send the exact wording, model, and symptom. It creates a clean content queue without opening public comments.</p><Link className="button button-primary" href="/submit-problem">Submit a problem</Link></div><div className="diagnosis-card" style={{ position: "static" }}><small>Good submissions include</small><h2>What happened, where it happened, and what changed.</h2><ol><li>Exact product and full model</li><li>Error code or visible symptom</li><li>What was already tried</li><li>What happened immediately before the issue</li></ol></div></div></section>
    </>
  );
}
