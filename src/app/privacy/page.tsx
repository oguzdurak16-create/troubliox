  ...
  return (
    <>
      ...
      <article className="container prose-page">
        ...
        <p>The contributor identifier is used to recognize a returning browser when structured product-experience data is submitted, so one browser does not repeatedly inflate an owner count. It is not an advertising identifier. When a product contribution is sent, Troublio transforms that identifier into a product-specific one-way hash before storing the contribution; the browser identifier itself is not stored with the experience record. If local storage is unavailable, a privacy-preserving technical fallback may be used for deduplication.</p>

        <h2>Structured contributions</h2>
        <p>When a visitor submits a product experience, missing-product request, troubleshooting outcome or problem request, the selected structured fields are transmitted to Troublio so they can be aggregated, deduplicated, reviewed or used to improve the site. Free-text contact information and links are not requested for these flows. Product experience pages publish aggregate statistics rather than the visitor&apos;s raw contribution record.</p>
